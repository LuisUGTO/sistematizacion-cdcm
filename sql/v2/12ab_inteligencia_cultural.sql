-- ============================================================================
-- ETAPA 7.7.2 - INTELIGENCIA CULTURAL
-- Capa estratégica de solo lectura; respeta RLS y el alcance institucional.
-- ============================================================================
...
BEGIN
  IF to_regclass('v2.registros') IS NULL
     ...
     OR to_regprocedure('v2.rpc_dashboard_directivo(integer,uuid,uuid,uuid)') IS NULL THEN
    RAISE EXCEPTION 'PRECONDICION: faltan el núcleo V2 o el Dashboard Directivo.';
  END IF;
END;
$$;
...
BEGIN
  ...
  v_base := v2.rpc_dashboard_directivo(p_anio, p_unidad, p_programa, p_municipio);
  ...
  RETURN COALESCE(v_base, '{}'::JSONB) || COALESCE(v_extra, '{}'::JSONB)
    || jsonb_build_object('modulo', 'INTELIGENCIA_CULTURAL', 'version', '7.7.2');
END;
$function$;

COMMENT ON FUNCTION v2.rpc_inteligencia_cultural(INTEGER, UUID, UUID, UUID) IS
'Vista estratégica de solo lectura para coordinación, supervisión y dirección. Respeta RLS.';

REVOKE ALL ON FUNCTION v2.rpc_inteligencia_cultural(INTEGER, UUID, UUID, UUID)
FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION v2.rpc_inteligencia_cultural(INTEGER, UUID, UUID, UUID)
TO authenticated;

INSERT INTO v2.schema_migrations(version, descripcion)
VALUES ('2.1.12ab', '12ab_inteligencia_cultural.sql - Capa estratégica de Inteligencia Cultural.')
ON CONFLICT (version) DO NOTHING;

COMMIT;

SELECT
  to_regprocedure('v2.rpc_inteligencia_cultural(integer,uuid,uuid,uuid)') IS NOT NULL AS rpc_existe,
  has_function_privilege('authenticated', 'v2.rpc_inteligencia_cultural(integer,uuid,uuid,uuid)', 'EXECUTE') AS authenticated_execute,
  NOT has_function_privilege('anon', 'v2.rpc_inteligencia_cultural(integer,uuid,uuid,uuid)', 'EXECUTE') AS anon_bloqueado,
  (SELECT count(*) FROM v2.schema_migrations WHERE version = '2.1.12ab') AS migracion;

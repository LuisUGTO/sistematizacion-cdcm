...
-- 12ac_dashboard_universo.sql
-- Etapa 7.7.3C: universos oficial y operativo para Inteligencia Cultural
...
--
-- CREA:
--   v2.rpc_dashboard_directivo_universo(integer, uuid, uuid, uuid, text)
--
-- SEGURIDAD:
...
CREATE OR REPLACE FUNCTION v2.rpc_dashboard_directivo_universo(
...
parametros AS (
  SELECT
    ...
    CASE
      WHEN upper(COALESCE(p_modo, 'OPERATIVO')) = 'OFICIAL' THEN 'OFICIAL'
      ELSE 'OPERATIVO'
    END AS modo
),
...
COMMENT ON FUNCTION v2.rpc_dashboard_directivo_universo(
  INTEGER,
  UUID,
  UUID,
  UUID,
...
'Dashboard directivo filtrable por universo OFICIAL u OPERATIVO. Respeta RLS mediante SECURITY INVOKER.';
...
REVOKE ALL ON FUNCTION v2.rpc_dashboard_directivo_universo(
  INTEGER,
  UUID,
  UUID,
  UUID,
...
GRANT EXECUTE ON FUNCTION v2.rpc_dashboard_directivo_universo(
  INTEGER,
  UUID,
  UUID,
  UUID,
...
VALUES (
  '2.1.12ac',
  '12ac_dashboard_universo.sql - Universos OFICIAL y OPERATIVO para Inteligencia Cultural.'
)
...
SELECT
  ...
  COALESCE(
    (
      ...
      WHERE n.nspname = 'v2'
        AND p.proname = 'rpc_dashboard_directivo_universo'
        AND pg_catalog.pg_get_function_identity_arguments(p.oid)
          = 'p_anio integer, p_unidad uuid, p_programa uuid, p_municipio uuid, p_modo text'
      LIMIT 1
    ),
    false
  ) AS security_invoker,
